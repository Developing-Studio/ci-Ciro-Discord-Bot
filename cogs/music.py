import asyncio
import re
import wavelink
import discord
import itertools
from discord.ext import commands
from typing import Union

RURL = re.compile(r'https?:\/\/(?:www\.)?.+')


class QueueControl:
    def __init__(self, bot, guild_id):
        self.bot = bot
        self.guild_id = guild_id
        self.channel = None

        self.next = asyncio.Event()
        self.queue = asyncio.Queue()

        self.volume = 50
        self.now_playing = None

        self.bot.loop.create_task(self.controller_loop())

    async def controller_loop(self):
        await self.bot.wait_until_ready()

        player = self.bot.wavelink.get_player(self.guild_id)
        await player.set_volume(self.volume)

        while True:
            if self.now_playing:
                await self.now_playing.delete()

            self.next.clear()

            song = await self.queue.get()
            await player.play(song)
            self.now_playing = await self.channel.send(f'In Riproduzione: `{song}`')

            await self.next.wait()


class music(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.controllers = {}

        if not hasattr(bot, 'wavelink'):
            self.bot.wavelink = wavelink.Client(bot=self.bot)

        self.bot.loop.create_task(self.start_nodes())

    async def start_nodes(self):
        await self.bot.wait_until_ready()
        LavaLinkServer = '127.0.0.1'
        # LavaLinkServer = '192.168.1.122'
        nodes = await self.bot.wavelink.initiate_node(host=LavaLinkServer, port=2333,
                                                      rest_uri="http://" + LavaLinkServer + ":2333",
                                                      password="IT | Kewai#9029", identifier=self.bot.user, region='eu_central')

        nodes.set_hook(self.on_event_hook)

    async def on_event_hook(self, event):
        if isinstance(event, (wavelink.TrackEnd, wavelink.TrackException)):
            controller = self.get_controller(event.player)
            controller.next.set()

    def get_controller(self, value: Union[commands.Context, wavelink.Player]):
        if isinstance(value, commands.Context):
            gid = value.guild.id
        else:
            gid = value.guild_id

        try:
            controller = self.controllers[gid]
        except KeyError:
            controller = QueueControl(self.bot, gid)
            self.controllers[gid] = controller

        return controller

    @commands.command(name='connect', aliases=['join', 'j', 'entra'], description='Serve a connettere il bot a un canale vocale')
    async def connect_(self, ctx, *, channel: discord.VoiceChannel = None):
        """Connect to a valid voice channel."""
        if not channel:
            try:
                channel = ctx.author.voice.channel
            except AttributeError:
                raise discord.DiscordException(
                    'Non ti vedo in nessun canale vocale. Specifica in quale canale devo entrare!')

        player = self.bot.wavelink.get_player(ctx.guild.id)
        await ctx.send(f'Mi sto collegando in  **`{channel.name}`**', delete_after=15)
        await player.connect(channel.id)

        controller = self.get_controller(ctx)
        controller.channel = ctx.channel

    @commands.command(aliases=['p'], description='Serve a riprodurre o a mettere in coda un brano')
    async def play(self, ctx, *, query: str):
        if not RURL.match(query):
            query = f'ytsearch:{query}'

        tracks = await self.bot.wavelink.get_tracks(f'{query}')

        if not tracks:
            return await ctx.send('Non trovo nessuna canzone!')

        player = self.bot.wavelink.get_player(ctx.guild.id)
        if not player.is_connected:
            await ctx.invoke(self.connect_)

        track = tracks[0]

        controller = self.get_controller(ctx)
        await controller.queue.put(track)
        await ctx.send(f'Ho aggiunto {str(track)} alla coda.')

    @commands.command(name='pausa', aliases=['pause'], description='Serve a mettere in pausa un brano')
    async def pause(self, ctx):
        player = self.bot.wavelink.get_player(ctx.guild.id)
        if not player.is_playing:
            return await ctx.send('Non sto riproducendo brani!', delete_after=15)

        await ctx.send('Ho messo in pausa il brano!', delete_after=15)
        await player.set_pause(True)

    @commands.command(aliases=['r', 'riprendi'], description='Serve a riprendere la riproduzione di un brano')
    async def resume(self, ctx):
        player = self.bot.wavelink.get_player(ctx.guild.id)
        if not player.paused:
            return await ctx.send('Attualmente non ho brani in pausa!', delete_after=15)

        await ctx.send('Riprendo la riproduzione!', delete_after=15)
        await player.set_pause(False)

    @commands.command(aliases=['s', 'skippa'], description='Serve a passare al prossimo brano in coda')
    async def skip(self, ctx):
        player = self.bot.wavelink.get_player(ctx.guild.id)

        if not player.is_playing:
            return await ctx.send('Non sto riproducendo brani!', delete_after=15)

        await ctx.send('Passo al brano sucessivo!', delete_after=15)
        await player.stop()

    @commands.command(aliases=['vol'], description='Serve ad abbassare o alzare il volume')
    async def volume(self, ctx, *, vol: int):
        """Set the player volume."""
        player = self.bot.wavelink.get_player(ctx.guild.id)
        controller = self.get_controller(ctx)

        vol = max(min(vol, 1000), 0)
        controller.volume = vol

        await ctx.send(f'Ho impostato il volume a `{vol}`')
        await player.set_volume(vol)

    @commands.command(aliases=['np', 'current', 'nowplaying', 'come', 'titolo'], description='Ti dice il titolo del brano in riproduzione')
    async def now_playing(self, ctx):
        player = self.bot.wavelink.get_player(ctx.guild.id)

        if not player.current:
            return await ctx.send('Non sto riproducendo brani!')

        controller = self.get_controller(ctx)
        embed = discord.Embed(title=player.current.title, colour=0xd629c9)
        embed.set_thumbnail(url=player.current.thumb)

        minutes = round((player.current.length / 1000) / 60)

        embed.add_field(name="Durata", value=f"{minutes} minuti")

        await controller.now_playing.delete()

        controller.now_playing = await ctx.send(embed=embed)

    @commands.command(aliases=['q', 'coda', 'c'], description='Serve a vedere la coda di riproduzione')
    async def queue(self, ctx):
        player = self.bot.wavelink.get_player(ctx.guild.id)
        controller = self.get_controller(ctx)

        if not player.current or not controller.queue._queue:
            return await ctx.send('Non ci sono canzoni in coda!', delete_after=20)

        upcoming = list(itertools.islice(controller.queue._queue, 0, 5))

        fmt = '\n'.join(f'**`{str(song)}`**' for song in upcoming)
        embed = discord.Embed(title=f'Brani sucessivi - {len(upcoming)}', description=fmt)

        await ctx.send(embed=embed)

    @commands.command(aliases=['disconnect', 'dc', 'disconnetti', 'leave', 'l'])
    async def stop(self, ctx):
        """Stop and disconnect the player and controller."""
        player = self.bot.wavelink.get_player(ctx.guild.id)

        try:
            del self.controllers[ctx.guild.id]
        except KeyError:
            await player.disconnect()
            return await ctx.send('Non cè niente da stoppare.')

        await player.disconnect()
        await ctx.send('Mi sono disconnesso con successo.', delete_after=20)

    @commands.command(aliases=['yt'], description='Cerca un brano su yt')
    async def ytsearch(self, ctx, *, query):
        embed = discord.Embed(title=f"Ho trovato questi risultati per: {query}", color=0xd629c9)
        query = await self.bot.wavelink.get_tracks(f'ytsearch: {query}')

        for track in query[0:9]:
            embed.add_field(name=f"{track}", value=u'\u200b', inline=False)

        await ctx.send(embed=embed)


def setup(bot):
    bot.add_cog(music(bot))
