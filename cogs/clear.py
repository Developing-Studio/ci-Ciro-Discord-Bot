...
class clear(commands.Cog):
    ...
    async def cancella(self, ctx, amount: int):
        if amount < 201:
            ...
            embed = discord.Embed(title=":warning: | Stai cercando di cancellare piu di 200 messaggi!",
    ...
    async def until_subcommand(self, ctx, ID: discord.Message):
        try:
            await ctx.channel.fetch_message(ID.id)
    ...
    async def clear_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            embed.add_field(name="Quanti messaggi devo cancellare?",
